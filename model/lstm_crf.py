...
class BLSTM(object):
    ...
    def build_graph(self, args, max_len, chn_size, label_dic):
        ...
        with self.graph.as_default():
            ...
            # reshape the output_fc for CRF
            outputs = tf.reshape(outputs_fc, [args.batch_size, max_len, args.num_classes])
           
            # CRF 
            seq_loss, trans_params = tf.contrib.crf.crf_log_likelihood(\
                                outputs, self.targets, self.seq_len)
            ...
            self.prediction , viterbi_score= tf.contrib.crf.crf_decode(outputs, trans_params, self.seq_len)
